...
{
	AMyProject2Character* overlappedActor = Cast<AMyProject2Character>(OtherActor);
	...
	if (overlappedActor)
	{
		...
		if (dmiMat)
		{
			...
			if (colorP)
			{
				UNiagaraComponent* particleComp = UNiagaraFunctionLibrary::SpawnSystemAttached(colorP, OtherComp, NAME_None, FVector(0.0f, 0.f, 0.f), FRotator(0.f), EAttachLocation::KeepRelativeOffset, true);
